...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            updateOnline: function (id, flag) {
                var config = {
                    url: table.options.updateOnlineUrl,
                    type: "post",
                    dataType: "json",
                    data: {"id": id, "online": flag},
                    beforeSend: function () {
                        $.modal.loading("正在处理中，请稍候...");
                        $.modal.disable();
                    },
            ...
            test: function (id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.testCaseUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openFull("测试" + table.options.modalName, url);
                } else {
                    $.modal.openFull("测试" + table.options.modalName, $.operate.testUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function (id) {
            ...
            testUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.testCaseUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
